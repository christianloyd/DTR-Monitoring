...
<html lang="en" class="h-full">
 ...
 <body class="bg-slate-50 font-sans text-slate-800 antialiased overflow-hidden h-full text-sm">
  ...
  <div class="flex h-screen w-full overflow-hidden">
   ...
   ══════════════════════════════════════════════════ -->
   <aside id="sidebar"
    class="fixed inset-y-0 left-0 z-50 w-[260px] bg-slate-900 text-white flex flex-col shadow-2xl 
           transform -translate-x-full lg:translate-x-0 lg:static transition-transform duration-300 ease-in-out border-r border-slate-800">
   ...
   <!-- ══════════════════════════════════════════════════
        MAIN AREA 
   ══════════════════════════════════════════════════ -->
   <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50 relative">

    <!-- Top Header -->
    <header class="bg-white/80 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-8 py-4 flex items-center justify-between flex-shrink-0 z-10 sticky top-0">
     <div class="flex items-center gap-4">
      <!-- Hamburger Menu (Mobile Only) -->
      <button onclick="openSidebar()" class="p-2 -ml-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors lg:hidden focus:outline-none">
       <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
       </svg>
      </button>
      <div>
       <h1 id="page-title" class="text-xl font-extrabold text-slate-800 tracking-tight">Overview</h1>
       <p id="page-subtitle" class="text-[13px] font-medium text-slate-500 mt-0.5">Welcome back to OJT DTR Admin</p>
      </div>
     </div>
     
     <div class="flex items-center gap-4">
      <div class="hidden sm:flex flex-col items-end mr-1">
       <p class="text-sm font-bold text-slate-800 leading-none"><?= htmlspecialchars($user['name']) ?></p>
       <p class="text-[10px] text-slate-500 uppercase tracking-widest font-bold mt-1.5 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block"></span>Administrator</p>
      </div>
      <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-slate-100 to-slate-200 border border-slate-200 flex items-center justify-center text-slate-600 shadow-sm">
